...
- 支持通过WEB界面手动触发测试推送，并可按需开启推送请求/响应调试日志
- 支持长短信自动合并（30秒超时）
- 支持管理员短信远程发送短信和重启设备
...
如果串口提示“配置有效，发送启动通知...”，但实际没有收到推送，可以进入工具页面进行排查：
...
- 点击“开启推送调试”，打开本次排查日志
- 点击“测试推送”，手动发送一条测试消息到所有已启用且配置完整的推送通道
- 页面日志区和串口都会输出本次推送的请求方式、URL、请求体、响应码和响应内容
- 如果没有发出请求，也会提示跳过原因，例如 WiFi 未连接、没有启用且配置完整的推送通道、缺少 URL、自定义模板为空等
- 排查结束后点击“关闭推送调试”，避免后续串口日志被推送调试信息污染

测试推送使用的发送者为 `TEST_PUSH`，内容为“这是一条推送通道测试消息”，用于确认当前推送通道是否可达。
